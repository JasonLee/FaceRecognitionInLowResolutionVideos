...
class YOLO(object):
    ...
    def detect_image_new(self):
        name_list = open(self.name_list)
        ...
        with tqdm(total=count) as pbar:
            for img_name in name_list:
                ...
                image = Image.open(self.input_dir+'/'+img_name)
                if self.model_image_size != (None, None):
                    assert self.model_image_size[
                               0] % 32 == 0, 'Multiples of 32 required'
                    assert self.model_image_size[
                ...
                try:
                    x = out_boxes[0]
                    region = image.crop((x[1],x[0],x[3],x[2]))
                    region.save(self.output_dir+'/'+img_name)
                except:
                    print('Can\'t detect faces in'+ img_name)
                pbar.update(1)
        self.close_session()
